...
def read_mrqa_examples(input_file, is_training, debug=False):
    def is_whitespace(c):
    ...
    ###################### Make Examples ######################
    examples = []
    for item in tqdm(unproc_data):
        # 1. Get Context
        paragraph_text = item["context"]
        special_tokens = ['[TLE]', '[PAR]', '[DOC]']
        
        for special_token in special_tokens:
            paragraph_text = paragraph_text.replace(special_token, "[CLS]")
